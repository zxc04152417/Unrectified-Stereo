...
def readDispKITTI(filename):
    ...
    valid = abs(disp) > 0.0    #这个地方存在问题，这个valid只取了大于0的，直接把垂直视差过滤掉了
    return disp, valid
...
def load_matrices(txt_path):
    ...
    for line in lines:
        ...
        if line.startswith('H_total:'):
            # 提取H_total数据
            values_str = line.replace('H_total:', '').strip()
            values = [float(x) for x in values_str.split()]
            H_total = np.array(values).reshape(3, 3)

        elif line.startswith('H_inv:'):
            # 提取H_inv数据
            values_str = line.replace('H_inv:', '').strip()
            values = [float(x) for x in values_str.split()]
            H_inv = np.array(values).reshape(3, 3)

    if H_total is None or H_inv is None:
        raise ValueError("文件格式错误，未找到H_total或H_inv")

    #print(f"矩阵已读取: {txt_path}")
    return H_total, H_inv

def load_obj(filename):
    disp = cv2.imread(filename, cv2.IMREAD_ANYDEPTH)
       #这个地方存在问题，这个valid只取了大于0的，直接把垂直视差过滤掉了
    return disp