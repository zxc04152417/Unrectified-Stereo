...
def demo_with_synthetic_data_v2(test_large_offset=False):
    ...
    names = sorted(glob(dir_left))
    for name in names:
        pL = name
        pR = name.replace('left', 'right')
        disp_root = name.replace('frames_finalpass', 'disparity').replace('.png', '.pfm')
        if not (os.path.isfile(pL) and os.path.isfile(pR) and os.path.isfile(disp_root)):
            continue

        left  = cv2.imread(pL)
        right = cv2.imread(pR)

        Right_rm_root = create_custom_folder(pR,      'im')
        disp_h_root   = create_custom_folder(disp_root,'disp_h')
        disp_v_root   = create_custom_folder(disp_root,'disp_v')
        H_inv_root    = create_custom_folder(disp_root,'H_inv')

        K_right = get_intrinsics_from_path(pR)
        disp    = readPFM(disp_root)
        ...
        cv2.imwrite(os.path.join(Right_rm_root, os.path.basename(pR)), right_out)

        # 生成 (h,v) 并落盘（pfm）
        h_occ, v_occ = sim.compute_disparity_field()
        h_occ = np.float32(h_occ)
        v_occ = np.float32(v_occ)
        writePFM(os.path.join(disp_h_root, os.path.basename(disp_root)), h_occ)
        writePFM(os.path.join(disp_v_root, os.path.basename(disp_root)), v_occ)
