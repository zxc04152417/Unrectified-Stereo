...
def write_pfm(file_path, image, scale=1):
    import os
    assert type(file_path) is str and type(image) is np.ndarray and \
           os.path.splitext(file_path)[1] == ".pfm"

    with open(file_path, 'wb') as f:
        # 确保是2D图像
        if len(image.shape) == 3:
            raise Exception('This writePFM implementation only supports 2D arrays (grayscale images)')
        ...
        # 翻转并转换为float32
        image = np.flip(image, axis=0).astype(np.float32)
...
def read_pfm(file_path):
    ...
    shape = (height, width, 3) if color else (height, width) #这里先写的高度再写的宽度
    ...
    with open(calib_path, 'r') as f:
        for line in f:
            ...
            if '=' in line:
                ...
                if key in ['baseline', 'width', 'height', 'ndisp', 'doffs']:
                    calib_data[key] = float(value)
                elif key.startswith('cam'):
                    # 解析相机内参矩阵 cam0=[541.764 0 553.869; 0 541.764 232.396; 0 0 1]
                    # 去掉方括号
                    matrix_str = value[1:-1]  # 去掉 [ 和 ]
                    # 按分号分割行
                    ...
                    for row in rows:
                        ...
                        K.append(row_values)
                    calib_data[key] = np.array(K, dtype=np.float32)
...
def demo_with_synthetic_data_v2(test_large_offset=False):
    # 根路径（按需改）
    training_dir = "/media/kemove/zxc/stereo/EH3D_ALL/EH3D/two_view_training"  # 修改为你的实际路径
    gt_dir = "/media/kemove/zxc/stereo/EH3D_ALL/EH3D/two_view_training_gt"  # 修改为你的实际路径

    scene_folders = [d for d in os.listdir(training_dir)]
    scene_folders.sort()


    for scene_name in scene_folders:
        scene_path1 = os.path.join(training_dir, scene_name)
        gt_path1 = os.path.join(gt_dir, scene_name)
        scene_path = os.path.join(scene_path1, 'im0.png')
        scene_path_right = os.path.join(scene_path1, 'im1.png')
        calib_path = os.path.join(scene_path1, 'calib.txt')
        gt_path = os.path.join(gt_path1, 'disp0GT.pfm')


        left  = cv2.imread(scene_path)
        right = cv2.imread(scene_path_right)

        calib_data = parse_calib_file(calib_path)
        height, width = left.shape[:2]

        K_left = calib_data['cam0']
        K_right = calib_data['cam1']
        K_left = K_left[:, :3]
        K = K_right[:, :3]
        ...
        img1_re_path = os.path.join(scene_path1, 'img1_re.png')
        cv2.imwrite(img1_re_path, right_out)
        ...
        h_occ1 = h_occ.copy()
        v_occ1 = v_occ.copy()
        h_occ[h_occ == 0] = np.inf
        v_occ[v_occ == 0] = np.inf

        # saveDispKITTI(h_occ, os.path.join(dir_h,     name.replace('.png', '.tiff')))
        # saveDispKITTI(v_occ, os.path.join(dir_v,     name.replace('.png', '.tiff')))

        level_path = os.path.join(gt_path1, 'level.pfm')
        vertical_path = os.path.join(gt_path1, 'vertical.pfm')

        write_pfm(level_path, h_occ.astype(np.float32))
        write_pfm(vertical_path, v_occ.astype(np.float32))
        ...
        calib_invs_path = os.path.join(scene_path1, 'img1_re.txt')
        save_matrices(sim.H_total_center, sim.H_inv_center, calib_invs_path)


        # 逆验证（由 (h,v) -> rectified d）
        recovered = sim.inverse_transform_verification(h_occ, v_occ)
        errs = sim.calculate_errors(recovered)
        print(f"[verify] {scene_name}: mean={errs['mean_error']:.6f}, median={errs['median_error']:.6f}, "
